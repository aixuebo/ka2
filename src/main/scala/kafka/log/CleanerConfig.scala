 ...
 */
case class CleanerConfig(val numThreads: Int = 1, //多少个线程去执行清理工作
                         ...
                         val maxIoBytesPerSecond: Double = Double.MaxValue,//一个阀门,控制所有的清理线程的IO
                         val backOffMs: Long = 15 * 1000,
                         val enableCleaner: Boolean = true,//默认是true,表示打开清理log日志功能
